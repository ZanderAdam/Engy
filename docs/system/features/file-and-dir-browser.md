...
**Live change notifications** are subscription-driven. Browser surfaces that consume `FILE_CHANGE` events (open doc dock tabs via `useWatchPaths` in `web/src/contexts/events-context.tsx`, and the task pages' `plans/` directories) declare the absolute paths they display; the server unions them per workspace (`web/src/server/ws/watch-subscriptions.ts`) and syncs the union to the daemon as `WATCH_PATHS_SYNC`. The daemon's `SpecWatcher` (`client/src/watcher.ts`, chokidar polling at 1-second intervals to avoid macOS libuv/PTY interference) watches exactly those paths — files or directories — pruning dot-segments and `node_modules` beneath subscribed directories, and watches nothing when no surface is open. On `add`, `change`, or `unlink` it sends `FILE_CHANGE` to the server; `adddir`/`unlinkdir` are silently dropped by `mapEventType`. The server's `handleFileChange` relays the event to `broadcastFileChange` (`web/src/server/ws/broadcast.ts`), which sends it to every WebSocket in `state.fileChangeListeners` whose `readyState === OPEN`. Browser clients connect to `/ws/events` (`web/src/server/ws/events-server.ts`), which registers and deregisters sockets on connection and close. Events missed while a watcher is (re)created are covered by fresh-on-mount queries and the editor's manual "Reload from disk" button.
...
| FR-FILES-190 | WHEN the daemon's `SpecWatcher` detects an `add`, `change`, or `unlink` event on a subscribed path (or beneath a subscribed directory, excluding dot-segments and `node_modules`), the system SHALL send a `FILE_CHANGE` WebSocket message containing `{ workspaceSlug, path, eventType }` to the server; paths absent from the latest `WATCH_PATHS_SYNC` (including workspaces with empty path sets) SHALL emit nothing. |
| FR-FILES-200 | WHEN the server receives a `FILE_CHANGE` message from the daemon, the system SHALL broadcast a `FILE_CHANGE` event to all open `/ws/events` browser connections. (Subscription aggregation is specified in the websocket-daemon-protocol area as FR-WS-190/FR-WS-200.) |
