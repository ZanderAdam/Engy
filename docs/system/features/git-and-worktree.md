...
A terminal panel's top bar links to the branch review of the repo the session works in: `resolveDiffTarget` (`web/src/components/terminal/terminal-diff-target.ts`) maps the session's working directory — `agentCwd` once the agent has moved, else the spawn directory — to a repo — through `worktree.listGrouped` for a worktree, otherwise to the innermost `workspace.repos` entry containing it — and `branchDiffHref` builds `/w/<ws>/projects/<project>/diffs?diffView=branch&diffRepo=<repoDir>` plus `wt=<branch>` for a worktree. The Diffs page seeds its repo and view mode from those params, and re-reads them when an already-open tab is retargeted.
...
| FR-GIT-490 | WHEN a terminal session works inside a known repo, the system SHALL offer a link from its panel to that repo's branch review, naming the repo and the branch the way the diffs surface's own selectors name them; the directory resolved SHALL be the one the agent reports itself in, so a session that entered a worktree links to that worktree's review; WHEN git reports a repo the workspace knows, the link SHALL carry that repo and git's branch; OTHERWISE the link SHALL carry the innermost workspace repo containing the directory, or the workspace's only repo, and SHALL carry no branch, because the branch git reported belongs to a different repo; IF no repo can be named, THEN no link SHALL be offered. |
...
| FR-GIT-540 | WHEN the diffs surface reloads what it is showing, every open file's patch is refetched at once; the system SHALL split those calls across requests small enough that no request line reaches the server's header limit, because a rejected batch carries no body and every call in it would report a parse failure in place of its diff. |
| FR-GIT-530 | WHEN a terminal session is listed or its panel is shown, the branch displayed SHALL be the one git reports for the directory the session is in now, not the branch recorded when the session was created, so a restored session and a session that moved into a worktree both read correctly; WHILE git has not answered, the recorded branch SHALL stand. |
| FR-GIT-520 | WHEN the system resolves the branch of a directory, it SHALL also report the directory of the repo that owns it, resolved through the repo's common git directory so a worktree names the repo it was created from rather than itself; IF the directory belongs to no repo, THEN no repo directory SHALL be reported. |
| FR-GIT-500 | WHEN the diffs surface is opened with its own repo, branch or view-mode parameter (`diffRepo` / `diffBranch` / `diffView`, named apart from other sections' parameters because the project nav copies every parameter onto its section links), it SHALL start on that repo, worktree and mode instead of its defaults, resolving the repo by name — or by path, which a link SHALL use instead whenever two of the workspace's repos share a directory name — and the branch to the worktree its own selector offers for that branch; WHILE that branch has no known worktree the reader's own selection SHALL stand; WHEN the reader changes the repo or worktree the parameters SHALL stop overriding them; and the surface SHALL follow the parameters when they change while it stays open. |
