...
| FR-MCP-220 | WHEN `diff_review_comment` is called, the system SHALL insert a comment thread at `diff://<repoDir>#<branch>/<filePath>`, the branch read from the daemon rather than from the call with `workspaceId: null` and metadata `{ type: 'diff', source: 'agent', lineNumber, codeLine, side, severity }` — `side` defaulting to `modified`, plus `evidence` when the caller supplies one and omitted entirely otherwise — whose single comment body carries the finding, its failure scenario, and the suggested fix when one is given, and SHALL return the new thread id. |
