...
| FR-SEARCH-016 | WHEN building a snippet for a search result, the system SHALL exclude YAML frontmatter from the displayed snippet text in every mode (lex, vector, hybrid). |
| FR-SEARCH-017 | WHERE the mode is lex and the query returns no results, the system SHALL progressively relax it — stripping stop words, then dropping the shortest remaining tokens — until a search returns results or relaxation is exhausted. |
| FR-SEARCH-018 | WHEN a workspace is indexed (incremental update or full reindex), the system SHALL remove vector embeddings that no longer reference an active document. |
