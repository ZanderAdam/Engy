...
| FR-PRMON-190 | WHEN `listOpenPrs` returns at least one PR, the system SHALL resolve the gh viewer login via `gh api user` in the repo's directory and set `authoredByViewer` per PR by comparing it to the PR author; IF the identity cannot be resolved, `authoredByViewer` SHALL be null and `upsertPrs` SHALL retain the stored value rather than overwriting it. The PRs tab SHALL default its filter to `workspaces.prScope` (`'mine'` for new workspaces), SHALL let the user switch between mine and all for the current visit without changing the stored default, and IF the mine filter hides every open PR THEN it SHALL say so and offer to show all. |
