...
Each PR row shows: CI status pill (styled via `pr-helpers.ts`), check breakdown, review decision, comment count (conversation comments + bodied review submissions, hidden at zero), correlated agent session link, and an attention badge (`web/src/components/prs/pr-attention.ts`) with a shadcn Tooltip when `prs.attentionReason` is set. A Mine/Review toggle in the header filters the list by `prs.authoredByViewer` (`filterPrsByScope` in `pr-helpers.ts`): Mine shows PRs you authored, Review shows PRs that request your review; it starts from the workspace's `prScope` setting and a click overrides it for the current visit only — the stored default is changed in the workspace edit dialog's Container tab. A manual Refresh button calls `trpc.pr.refresh`. The list updates live via `useOnServerEvent('PR_CHANGE')`. Error display is two-tier (`web/src/components/prs/pr-errors.ts`): gh-not-installed and no-daemon collapse to a global banner (one binary, one daemon), while auth and other errors render as inline per-repo rows above the list — healthy repos keep listing.
...
Key columns: `repo`, `number`, `title`, `url`, `headBranch`, `headSha`, `author`, `isDraft`, `ciStatus` (`pending`/`passing`/`failing`/`unknown`), `checks` (JSON), `commentCount`, `authoredByViewer`, `reviewDecision`, `lastFailedHeadSha`, `autoFixAttempts`, `autoFixTotalAttempts`, `attentionReason`. There is no `state` column: `listOpenPrs` returns only open PRs, so rows absent from a fresh list (closed, or no longer yours or awaiting your review) are deleted outright.
...
`GH_PR_REVIEW_COMMENTS` fetches review comments via `gh api repos/{o}/{r}/pulls/{n}/comments --paginate --slurp` (slurp is required: plain `--paginate` emits one JSON array per page, causing `JSON.parse` to fail on multi-page results). `syncReviewComments` in `web/src/server/pr/review-sync.ts` imports them into `commentThreads` / `threadComments` keyed by deterministic ids (`gh-thread-{githubId}` / `gh-comment-{githubId}`), with `documentPath: diff://{repo}#{headBranch}/{path}` — keyed on the pull request's head branch, so an imported comment shows on the branch it was written against — and `metadata.source: 'github'`. Locally-resolved threads are never auto-unresolved. Comments deleted on GitHub are preserved locally.
...
| FR-PRMON-030 | The system SHALL persist open PRs in a `prs` table keyed by `(repo, number)` via `upsertPrs`; on re-poll the system SHALL update all fields, report material changes (`new`, `ciStatus`, `reviewDecision`, `commentCount`, `removed`), delete rows for PRs absent from the incoming list (no longer open), and perform all writes in a single transaction. |
...
| FR-PRMON-120 | The PRs tab SHALL list open PRs for all workspace repos ordered by `updatedAt` descending, showing per-PR CI status pill, check breakdown, review decision, comment count (hidden when zero), correlated agent session link, and a manual Refresh action. |
...
| FR-PRMON-180 | WHEN `listOpenPrs` is called, the system SHALL request the `comments` and `reviews` fields and derive `commentCount` as the number of conversation comments plus the number of review submissions with a non-empty body; `upsertPrs` SHALL persist it to `prs.commentCount` and report a `commentCount` material change when it differs from the stored value. |
| FR-PRMON-190 | WHEN `listOpenPrs` is called, the system SHALL run two `gh pr list --limit 100` queries in the repo's directory — `--author @me` and `--search review-requested:@me` — keep only `OPEN` results, list a PR returned by both once, and set `authoredByViewer` to true exactly for PRs from the author query. The PRs tab SHALL offer a Mine/Review toggle (mine = `authoredByViewer`, review = the rest), SHALL default it to `workspaces.prScope` (`'mine'` for new workspaces), SHALL let the user switch for the current visit without changing the stored default, and IF the selected scope is empty while the other is not THEN it SHALL say so and offer to switch. |
