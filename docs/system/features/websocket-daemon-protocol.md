...
When the daemon detects a filesystem change it sends a `FILE_CHANGE` message (`{ workspaceSlug, path, eventType }`); `handleFileChange` in `server.ts` relays it straight to `broadcastFileChange` (see below). What the daemon watches is subscription-driven: browser surfaces send `WATCH_SUBSCRIBE` snapshots over `/ws/events` — each message carries that socket's full current set of absolute paths grouped by workspace (replace semantics). The server guards every path (absolute, no `..` segments, under the workspace's docs dir), stores per-socket snapshots in `state.watchSubscriptions`, and pushes the per-workspace union to the daemon as `WATCH_PATHS_SYNC` (`web/src/server/ws/watch-subscriptions.ts`) — debounced 300 ms, deduped against the last sent payload, force-sent on daemon `REGISTER`, and recomputed when a browser socket closes or errors.
...
| ID | Requirement (EARS) |
|----|--------------------|
| FR-WS-010 | WHEN the daemon sends `REGISTER`, the system SHALL store the socket as `state.daemon`, store `payload.homeDir` (or `null` if absent) as `state.daemonHomeDir`, and respond with a `WATCH_PATHS_SYNC` message containing the current per-workspace union of subscribed watch paths (sent even when the union is empty). |
