...
| FR-TERMINAL-160 | WHILE the command center is open, the system SHALL reflect each session's daemon-tracked `activityState` on its list entry — including sessions no browser is currently rendering — and SHALL render the selected session as a live, interactive terminal without navigating away from the command center. |
