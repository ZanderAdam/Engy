...
| FR-TERMINAL-150 | WHEN the command center requests the global terminal list via `GET /api/terminal/sessions?all=1`, the system SHALL return every session in `terminalSessionMeta` (carrying `projectSlug`, `worktreeBranch`, and `activityState`), and the client SHALL group them by project then by worktree branch with project-less sessions collected in a trailing bucket. |
| FR-TERMINAL-160 | WHILE Command Center mode is enabled (a global toggle shared across all project tabs), the system SHALL show the global terminal list in the terminal sidebar of every project tab — reflecting each session's daemon-tracked `activityState`, including sessions no browser is currently rendering — and SHALL render the selected session as a live, interactive terminal while the project's own terminal sessions stay mounted underneath. |
