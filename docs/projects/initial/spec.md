...
- Server and client can communicate over WmilestonebSocket (local or network)
...
| FR-7.3  | The system shall provide a Memory tab with browser (filter by type, tags + search) and detail view (content editor, metadata display). All memories are workspace-scoped; there is no scope filter.                                                                                                                                                                                                                                                                                                                                                                        |
...
| FR-7.10 | The system shall surface unpromoted fleeting memories as promotion candidates on project completion. Deduplication against existing permanent memories happens per-candidate during the /engy:review-memories review phase, not as a bulk pre-distillation pass.                                                                                                                                                                                                                                                                                                           |
| FR-7.11 | The system shall propose system doc updates on project completion — agent proposes diffs that appear in the diff viewer for review (delivered by the `engy:write-sysdocs` refresh mode).                                                                                                                                                                                                                                                                                                                                                                                   |
| FR-7.12 | The system shall archive completed projects: compact (preserve plan content, milestones, groups, task structure, key decisions, final statuses), discard agent session state and execution logs. Fleeting memories are preserved on archive — they are workspace-scoped and user-managed via /engy:review-memories.                                                                                                                                                                                                                                                        |
| FR-7.13 | The system shall inject memory context at planning time via the `engy:research` subagent dispatched by `engy:plan` and `engy:milestone-plan`. The returned digest is folded directly into the plan document. When work is repo-local, the subagent accepts an optional `repo` filter to bias retrieval; no mandatory scope ordering is enforced — retrieval relevance is governed by qmd hybrid ranking.                                                                                                                                                                   |
| FR-7.14 | The system shall provide a system-docs bootstrap capability (the `engy:write-sysdocs` init mode) that reads the codebase via the client connection and proposes initial system docs.                                                                                                                                                                                                                                                                                                                                                                                       |
| FR-7.15 | The system shall provide an `engy:write-sysdocs` Claude Code skill that authors and edits system docs — unifying initial bootstrap (FR-7.14), project-completion refresh (FR-7.11), and interactive per-doc editing; feature-area docs under `system/features/` are delegated to the `engy:feature-docs` skill.                                                                                                                                                                                                                                                            |
| FR-7.16 | All permanent memories are workspace-scoped. The `repo` field is optional provenance metadata indicating which codebase a memory concerns; it is available as an explicit filter but is not an automatic retrieval gate.                                                                                                                                                                                                                                                                                                                                                   |
| FR-7.17 | The system shall store permanent memories as markdown files with YAML frontmatter containing: id, type, subtype (decision/pattern/fact/convention/insight), title, repo (optional provenance), confidence, source, tags, keywords (low-level retrieval terms), themes (high-level conceptual terms), linkedMemories, scenarioIds (FR/scenario ID anchors), sources (paths to source records under memory/sources/ or memory/references/), supersededBy (nullable reference to newer memory), and timestamps. No scope field — all memories are workspace-scoped (FR-7.16). |
| FR-7.18 | The system shall support `engy validate` and `engy reindex` as terminal skills — not just CLI commands.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

**Exit Criteria:** Docs tab with system/shared docs. Memory tab with browser/editor. qmd hybrid search. Project completion with memory distillation and system doc proposals. Permanent memory schema with provenance fields (scenarioIds, sources). engy:ingest, engy:review-memories, engy:research subagent. Bootstrap, validate, reindex as terminal skills.
...
| 2026-06-01 | Aleks  | Reflected the system-docs skill consolidation: `bootstrap-sysdocs` + `propose-sysdocs` + `sysdoc-assistant` merged into `engy:write-sysdocs` (init / refresh / interactive modes), with feature-area docs owned by `engy:feature-docs`. Updated FR-4.7, FR-7.11, FR-7.14, FR-7.15.                                                                                                                                                                   | 0.6     |
| 2026-06-10 | Aleks  | M7 post-ship spec drift fixes: FR-7.3 drop scope filter; FR-7.10 dedup at review not distillation; FR-7.12 preserve fleetings on archive; FR-7.13 planning-time injection via engy:research, optional repo filter; FR-7.16 repo as provenance not scope enum; FR-7.17 drop scope field, add scenarioIds + sources fields; updated exit criteria.                                                                                                     | 0.7     |
