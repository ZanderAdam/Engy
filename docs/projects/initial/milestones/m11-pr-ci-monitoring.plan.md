...
4. The system shall poll PR status on a server-side interval per workspace repo via the daemon dispatch wrappers, upsert results, delete rows for PRs absent from the list (no longer open), and broadcast a `pr-change` server event on material change. Each repo is polled independently: a failure in one repo (not a GitHub remote, not authenticated for that host, gh error) is recorded as that repo's poll error and does not abort the cycle for remaining repos. *(user request: FR-11.2)* (FR-TG1.4)
...
3. **PR polling service** (depends on task 2)
   ...
   - Implements FR-TG1.4
   - Server-side `setInterval` (constant, ~60s; skip cycle when no daemon connected) iterating workspace repos with per-repo error isolation, reusing the router's upsert/correlate helper; delete rows for PRs absent from the list; add `pr-change` to the `ServerEvent` union + `broadcastPrChange` wrapper (template: `broadcastTaskChange`); timer on `AppState`, torn down on shutdown alongside existing timers. Verify: `pnpm blt`.
   - Type: ai. Important + urgent.
...
- Protocol: `GH_PR_LIST` op in `common/src/ws/protocol.ts`; daemon module `client/src/gh/` (injectable `GhRunner`, ciStatus derivation from mixed CheckRun/StatusContext rollups, coderWorkspace forwarded via the coder ssh runner pattern, typed errors not-installed / not-authenticated / real-error derived per call — the original `GH_AUTH_STATUS` preflight op was removed in the 2026-07-04 revision).
- Server: `dispatchGhPrList` in `web/src/server/ws/server.ts`; `prs` table + `pr` router (`list` with repo-scoped `agentSessions.branch` correlation, `refresh` sweeping all repos with per-repo typed errors); `upsertPrs` runs in a transaction and reports material changes.
...
1. The system shall detect CI status transitions to `failing` during polling and classify failures as mechanical (lint, type errors, test failures, missing deps) or non-mechanical, by heuristic on check names only — failure logs are dispatch context, not classifier input. *(user request: FR-11.3)* (FR-TG2.1)
2. The system shall fetch failing-check log excerpts via the daemon (`gh pr checks` / `gh run view --log-failed`) only when actually dispatching an auto-fix, to include as dispatch context. *(inferred: agent needs the error text)* (FR-TG2.2)
3. When a mechanical CI failure occurs on a PR correlated to an agent session, and the workspace setting `autoCiFix` is enabled, the system shall auto-dispatch a session resume in the existing worktree with the failure context — gated by daemon readiness and `maxConcurrency`, capped at 2 auto-fix attempts per (PR, head commit) plus 5 total per PR (new pushes reset the per-commit counter; without the total cap a failing fix-push loop would run forever). *(user request: FR-11.3; caps inferred: loop safety)* (FR-TG2.3)
...
## Post-completion KISS Revision (2026-07-04)

A KISS review after initial completion simplified four contracts; the code was reworked to match (requirement rows above already reflect the final state):

- **`GH_AUTH_STATUS` op removed** — typed not-installed / not-authenticated errors are derived from each gh call itself (ENOENT vs stderr); no preflight round-trip, and auth is naturally checked per repo (matters for multi-host, e.g. GitHub Enterprise remotes).
- **`prs.state` column removed; vanished PRs deleted** — `gh pr list` returns only open PRs and the tab shows only open PRs, so a soft-close path was dead weight.
- **Classification by check names only** — log-content heuristics were dropped as fragile; failed logs are fetched only when an auto-fix actually dispatches, as prompt context.
- **Per-repo error surfacing** — one failing repo shows an inline error on its rows while healthy repos keep listing; only no-daemon and gh-not-installed are global states.

Deliberately kept from the original implementation: the total cap of 5 auto-fix attempts per PR (in addition to the planned 2-per-head-SHA — new pushes reset the per-SHA counter, so the total cap is what actually bounds fix-push loops), and DB-load-on-mount for the PRs tab instead of a daemon sweep (the 60s poller makes a mount sweep redundant). Migration note: the branch's original migrations 0023–0026 were regenerated as a single migration after merging main (whose 0023/0024 were taken by the terminal-sessions work).
