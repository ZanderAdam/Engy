...
Boundary: link-walking is in-scope — the `engy:research` subagent traverses `linkedMemories`, `sources`, and `scenarioIds` frontmatter links at configurable depth with a total-node cap (not unbounded graph traversal). The excluded items are: a memory linking graph UI, an automatic background cleanup scheduler, cross-workspace search, and ad-hoc contradiction-flagging outside the ingestion path (contradiction surfacing happens during `engy:ingest`, not as a standalone always-on Interlocutor).
