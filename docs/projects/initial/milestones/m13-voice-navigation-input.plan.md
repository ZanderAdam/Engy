...
28. The system shall discard dictation heard while it is speaking. *(inferred: the mic stays open through a spoken answer, so the answer is otherwise transcribed and dictated back — the loop feeds itself. Barge-in is out of scope, so those words are dropped rather than queued)* (FR-TG2.28)
