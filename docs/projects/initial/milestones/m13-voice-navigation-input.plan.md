...
27. In conversation mode, the system shall submit dictated text to the focused terminal after a stretch of silence, restarting that window on each new segment, and shall enter and leave the mode by voice. *(source: user request — "just back and forth with claude, send after few s of silence"; a pause for thought must not send half a thought, and auto-submitting is a mode because dictating a long prompt in pieces needs those pauses to be free)* (FR-TG2.27)
28. While it is speaking, and for a short echo tail after, the system shall send silence in place of mic audio. *(inferred: the mic stays open through a spoken answer, so the answer is otherwise transcribed and dictated back — the loop feeds itself. A transcript arrives only after its speech ends, so the audio must be silenced at the source. Barge-in is out of scope, so words spoken over the answer are not heard)* (FR-TG2.28)
29. In conversation mode the microphone shall stay open across key release, and shall close only on an explicit end — a second key press, the listening control, or the spoken command. Other keys shall not end it, and shall cancel a pending auto-submit. Once it ends, the conversation's transcript shall not be shown again. *(revised after testing: the mode shipped with key release still ending the turn, so there was never a hands-free session for it to run in. The mode and the microphone are one state, so neither can be left on without the other)* (FR-TG2.29)
30. When several browser tabs show the same workspace, only the tab focused last shall play an agent's spoken reply; with no such tab, only a tab that has focus shall play it. *(revised after testing: every tab receives the broadcast, so three open tabs played each answer three times, overlapping)* (FR-TG2.30)
31. When the push-to-talk key is tapped and pressed again within 350 ms, the system shall start a conversation. *(user request)* (FR-TG2.31)
32. While spoken replies are on, the system shall keep a fixed one-line row under each terminal. When an agent speaks, the system shall show the full reply over the terminal above that row, until the user folds it to one line, dismisses it, or the agent speaks again. *(user request; revised after testing: a row that grew with each reply made the terminal shorter, and Claude Code redraws garbled when its terminal shrinks; a one-line reply hid the text)* (FR-TG2.32)
33. The system shall let the user choose the playback speed of spoken replies, from 0.75× to 2×, kept per browser. *(user request)* (FR-TG2.33)
34. The system shall let the user pick the voice for spoken replies from a fixed list of US and UK Piper voices, Amy by default, kept per browser. A voice shall download on first use, and picking one shall play a sample. *(user request)* (FR-TG2.34)
