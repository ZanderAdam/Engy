...
- `src/watcher.ts` — chokidar watcher over `{workspace.docsDir ?? ENGY_DIR/slug}/specs` and `.../projects` (only dirs that exist). Emits `FILE_CHANGE`.
...
Protocol catalog lives in `common/CLAUDE.md` (~60 message types in `@engy/common/src/ws/protocol.ts`).
...
- **WS tests** use real `WebSocketServer` instances on ephemeral ports with local `waitForConnection`/`waitForMessage` Promise helpers.
