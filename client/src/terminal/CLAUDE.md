...
- Each session owns an `@xterm/headless` terminal (`screen`, 10,000-line scrollback matching the browser xterm, resized alongside the PTY) fed every raw output chunk. On reconnect the daemon resizes the PTY and mirror to the `cols`/`rows` the server sends (the session's last known size — a mirror left at the spawn default serializes frames addressed to a geometry the pane does not have), flushes its write queue (`screen.write('', cb)`), and sends the `@xterm/addon-serialize` snapshot — never raw chunk history, whose cursor-relative TUI frames tear when replayed against a reset screen. Every session serializes `{ scrollback: SNAPSHOT_SCROLLBACK_LINES }` (5,000 of the mirror's 10,000 lines) — a resync after a refresh restores several browsers at once, so the depth is capped there. Do not key this on whether the session was spawned with a command: a full-screen program draws its frames on the alternate screen, which the serializer appends separately, so the normal buffer holds history either way.
