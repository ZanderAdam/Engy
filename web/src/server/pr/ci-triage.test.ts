...
describe('ci-triage', () => {
  describe('[FR-PRMON-060] detectFailureTransitions', () => {
    ...
    it('should ignore non-ciStatus changes', () => {
      const changes: MaterialChange[] = [
        makeChange({ type: 'removed', current: 'removed' }),
  ...
  describe('[FR-PRMON-070] classifyFailure', () => {
    ...
    it('should return non-mechanical for unknown check names', () => {
    ...
    it('should return non-mechanical when checks are empty', () => {
    ...
    it('should classify by check name only — mechanical-looking log content must not flip the result', () => {
      expect(classifyFailure([makeCheck('Production Deploy')])).toBe('non-mechanical');
    });
