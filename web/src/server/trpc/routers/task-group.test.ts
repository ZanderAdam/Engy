...
describe('task-group router', () => {
  ...
  describe('list', () => {
    it('should list task groups for a milestone', async () => {
      await caller.taskGroup.create({ milestoneId, name: 'Group A' });
      await caller.taskGroup.create({ milestoneId, name: 'Group B' });
      const result = await caller.taskGroup.list({ milestoneId });
  ...
  describe('update', () => {
    it('should update task group name', async () => {
      ...
      const updated = await caller.taskGroup.update({
        id: group.id,
        name: 'Updated Name',
      });
  ...
  describe('delete', () => {
    it('should delete an existing task group', async () => {
      ...
      await expect(caller.taskGroup.get({ id: group.id })).rejects.toThrow(
        'not found',
      );
    });
  });
