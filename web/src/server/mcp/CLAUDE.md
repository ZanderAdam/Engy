...
- Every MCP tool corresponds to a tRPC procedure with matching input shape, error semantics, and side effects. **When you change one, change the other.** Four exceptions, all agent-only by design:
  - `terminal_*` dispatch tools (`terminal-tools.ts`) — browsers manage the worker set via the `terminal` tRPC router; agents alone call dispatch/reply/collect/status.
  - `speak` (`voice-tools.ts`) — the browser already speaks its own voice-action answers locally, so its counterpart is `/api/voice/speak`, not a tRPC procedure.
  ...
  - `diff_review_*` tools (`diff-review-tools.ts`) — the browser writes the same `commentThreads` rows through `comment.createThread` and builds the `diff://` path itself in `use-diff-comments.ts`. These tools exist so an agent does not reconstruct that path and metadata shape by hand, so they carry a diff-shaped input rather than mirroring the comment router. Behaviour that belongs to comment threads generally still goes in both surfaces.
...
- `registerDiffReviewTools` (in `diff-review-tools.ts`) — `diff_review_comment`, `diff_review_summary`, `diff_review_list`. Writes review findings into the same `commentThreads`/`threadComments` tables the Diffs tab reads. Every row is written with `workspaceId: null`, because the Diffs tab queries `comment.listThreadsByPrefix` with no `workspaceSlug` and that filters on `workspaceId IS NULL` — a workspace-scoped thread is invisible there. Author fields (`source: 'agent'`, `agentSessionId`, `agentType`) are derived from the `/mcp/<terminalSessionId>` token, never from tool arguments, so `source` stays trustworthy as the discriminator that separates agent findings from the user's own comments.
