...
- `registerDiffReviewTools` (in `diff-review-tools.ts`) — `diff_review_comment`, `diff_review_summary`, `diff_review_list`. Writes review findings into the same `commentThreads`/`threadComments` tables the Diffs tab reads. Every row is written with `workspaceId: null`, because the Diffs tab queries `comment.listThreadsByPrefix` with no `workspaceSlug` and that filters on `workspaceId IS NULL` — a workspace-scoped thread is invisible there. The branch a review is filed against is read from the daemon (`dispatchGitBranch`) at call time, so a tool argument cannot misfile findings onto another branch's review. Author fields (`source: 'agent'`, `agentSessionId`, `agentType`) are derived from the `/mcp/<terminalSessionId>` token, never from tool arguments, so `source` stays trustworthy as the discriminator that separates agent findings from the user's own comments.
