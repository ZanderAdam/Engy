...
describe('[FR-EDITOR-150] mermaid visual editor model-ops', () => {
  it('[FR-EDITOR-150] adds a node with a unique id', () => {
    const { model: next, nodeId } = addNode(model('flowchart TD\n A --> B'));
    expect(nodeId).toBe('C');
    expect(next.nodes.map((n) => n.id)).toEqual(['A', 'B', 'C']);
  });

  it('[FR-EDITOR-150] adds a connected node with an edge from the source', () => {
  ...
  it('[FR-EDITOR-150] adds an edge but ignores self-loops and duplicates', () => {
  ...
  it('[FR-EDITOR-150] deletes a node along with its incident edges and membership', () => {
  ...
  it('[FR-EDITOR-150] deletes an edge by id', () => {
    const base = model('flowchart TD\n A --> B');
  ...
  it('[FR-EDITOR-150] updates node label and shape', () => {
  ...
  it('[FR-EDITOR-150] updates an edge label', () => {
    const base = model('flowchart TD\n A --> B');
    const next = updateEdgeLabel(base, base.edges[0].id, 'yes');
    expect(next.edges[0].label).toBe('yes');
  });

  it('[FR-EDITOR-150] duplicates a node into the same subgraph', () => {
  ...
  it('[FR-EDITOR-150] does not mutate the input model', () => {
    const base = model('flowchart TD\n A --> B');
