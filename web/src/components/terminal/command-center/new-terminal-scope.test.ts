...
describe('command center', () => {
  describe('cloneScopeForNewTerminal', () => {
    it('[FR-TERMINAL-170] clones the first terminal scope so the session lands in that project', () => {
    ...
    it('[FR-TERMINAL-170] strips a trailing ordinal so the manager can re-suffix cleanly', () => {
    ...
    it('[FR-TERMINAL-170] drops the task binding — a cloned terminal is not a task terminal', () => {
      const groups = groupTabsByWorktree([tab('a', { taskId: 42 })]);

      expect(cloneScopeForNewTerminal(groups)?.taskId).toBeUndefined();
    });

    it('[FR-TERMINAL-170] returns null when the group has no terminals', () => {
      expect(cloneScopeForNewTerminal([])).toBeNull();
    });

    it('[FR-TERMINAL-170] returns null for a non-project group — creation is scoped to project groups', () => {
      const groups = groupTabsByWorktree([
