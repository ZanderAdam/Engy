...
export function MobileHeader({ workspace, project, onOpenManageWorktrees }: MobileHeaderProps) {
  ...
  const basePath = `/w/${workspace.slug}/projects/${project.slug}`;

  const current = activeSection(pathname, basePath) ?? sections[0];
  const enabledSections = sections.filter((s) => !s.disabled);
  const sectionPosition = `${enabledSections.findIndex((s) => s.segment === current.segment) + 1}/${enabledSections.length}`;
  ...
  return (
    <header className="sticky top-0 z-30 flex shrink-0 flex-col bg-background">
      <MobileIdentityBar workspace={workspace} project={project} />
      ...
      <div className="flex h-11 items-stretch border-b border-border bg-background">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-full flex-1 justify-between rounded-none px-3">
          ...
          <DropdownMenuContent
            align="start"
            className="min-w-[12rem] md:w-auto w-[calc(100vw-1rem)]"
          >
            {sections.map((s) => {
              const Icon = s.icon;
