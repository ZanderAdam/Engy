...
describe('resolve', () => {
  describe('stripWakeWord', () => {
    it('[FR-TG2.13] should strip an exact wake word', () => {
      expect(stripWakeWord('Engy select project web', 'Engy')).toBe('select project web');
    });

    it('[FR-TG2.13] should strip a wake word followed by trailing punctuation', () => {
      expect(stripWakeWord('Engy, select project web', 'Engy')).toBe('select project web');
    });

    it('[FR-TG2.13] should strip the "Engie" phonetic variant', () => {
      expect(stripWakeWord('Engie select project web', 'Engy')).toBe('select project web');
    });

    it('[FR-TG2.13] should strip the spelled-out "N G" variant', () => {
      expect(stripWakeWord('N G select project web', 'Engy')).toBe('select project web');
    });

    it('[FR-TG2.13] should strip the "Energy" mishear', () => {
      expect(stripWakeWord('Energy select project web', 'Engy')).toBe('select project web');
    });

    it('should leave a transcript unchanged when it carries no wake word', () => {
      expect(stripWakeWord('select project web', 'Engy')).toBe('select project web');
    });

    it('should trim surrounding whitespace on an empty or wake-word-only transcript', () => {
      expect(stripWakeWord('  Engy,  ', 'Engy')).toBe('');
      expect(stripWakeWord('   ', 'Engy')).toBe('');
    });
    ...
    it.each([
      ...
      'okay let us begin',
      'unique names only please',
    ])(
      '[FR-TG2.13] should not mistake the ordinary leading word in "%s" for the wake word',
      (transcript) => {
        expect(stripWakeWord(transcript, 'Engy')).toBe(transcript);
      },
    );
  });
