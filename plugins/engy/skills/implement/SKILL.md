...
**Copy the plan's phases and deliverables in verbatim before adding your own tasks.** The failure mode is reading a plan, then writing a fresh task list that quietly drops its named steps. A step you decide not to do stays in the list as `skip: <reason>` — dropping it silently is not allowed, because the reason is what the user reviews.
...
- Task #4: "Run /engy:review, then the validation gate from Step 2" (blocked by #3) — CRITICAL, always required
...
**When the task is a bug fix**, the Red test must fail for the *confirmed* reason, not just fail. Trace the symptom to its mechanism before designing the fix, then ship the smallest change that mechanism justifies. A guard that silences a symptom you never traced is a hypothesis, not a fix — and code motivated by a hypothesis you later disproved gets reverted, not left in as insurance.
...
Grade every claim in this step on the evidence ladder, and end each check with one of **VERIFIED** / **NOT VERIFIED** / **INCONCLUSIVE**. Inconclusive is not a pass. Report the rung you reached, not just the outcome — "it builds" is rung 1 for anything the test suite does not cover.
...
5. Run any manual checks specified in project config. UI work reaches rung 5 (driven in a real browser) or it is not done.
...
- "the codegen step must run after any schema change, or the migration silently omits the new column"
- "the cache invalidates on a TTL, not on write — a read straight after a write can still return stale data"
- "the request/response map entry must be deleted in the response handler, or every timed-out request leaks"
...
2. **Validation gates** — each gate with its verdict, evidence rung, and artifact.
3. **Follow-ups** — remaining issues, deferred feedback, every `skip: <reason>` from Step 3, and anything left INCONCLUSIVE.

## Key Principles

- **Context before code.** Always gather task details before writing any code.
- **Evidence before claims.** Grade every claim on the evidence ladder and say where it stopped.
- **Task tracking.** Create session tasks for every step — nothing happens untracked, and nothing is dropped without a `skip: <reason>`.

## Additional resources

- For the evidence ladder and the three verdicts, see [references/evidence-ladder.md](references/evidence-ladder.md)
- For EARS-BDD mode, see [references/ears-bdd.md](references/ears-bdd.md)
